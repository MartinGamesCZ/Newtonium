...
console.log(
  `Building newtonium for ${
    os.platform() === "win32" ? "Windows x64" : "Linux"
  }...`
);

if (!existsSync(distPath)) mkdirSync(distPath);
if (!existsSync(path.join(os.homedir(), ".newtonium")))
  mkdirSync(path.join(os.homedir(), ".newtonium"));
...
async function download() {
  ...
  console.log(
    `https://github.com/MartinGamesCZ/Newtonium/releases/${tag}/download/newtonium`
  );

  if (existsSync(executablePath)) {
    console.log("Newtonium already exists. Skipping download.");
